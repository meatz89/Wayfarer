...
public interface IGameRuleEngine
{
    ...
    ActionValidationResult ValidateAction(Player player, ActionOption action);
    
    // Action outcomes
    ActionResult CalculateActionOutcome(Player player, ActionOption action);
    WorkReward CalculateWorkReward(ActionOption action, NPC npc);
    ...
    TravelEncounterType DetermineEncounterType(RouteOption route);
