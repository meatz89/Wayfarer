...
public class NPCLetterOfferService
{
    ...
    public bool ShouldNPCOfferLetters(string npcId)
    {
        ...
        int totalConnections = npcTokens.Values.Sum();
    ...
    public List<LetterOffer> GenerateNPCLetterOffers(string npcId)
    {
        ...
        List<ConnectionType> availableLetterTypes = GetAvailableLetterTypes(npc);
        ...
        foreach (ConnectionType letterType in availableLetterTypes)
        {
            // Check if player has enough tokens of this specific type with this NPC
            int tokensOfType = npcTokens.GetValueOrDefault(letterType, 0);
            ...
            if (tokensOfType >= GameRules.TOKENS_BASIC_THRESHOLD) // Basic threshold for any offers
            {
                ...
                LetterOffer offer = CreateLetterOffer(npc, letterType, tokensOfType);
                if (offer != null)
                {
                    offers.Add(offer);
                }
    ...
    {
        List<ConnectionType> availableTypes = new List<ConnectionType>();
    ...
    private LetterOffer CreateLetterOffer(NPC npc, ConnectionType letterType, int tokensOfType)
    {
        ...
        List<LetterTemplate> allTemplates = _letterTemplateRepository.GetTemplatesByTokenType(letterType);

        // Filter templates by token threshold - player must have enough tokens to unlock each category
        List<LetterTemplate> availableTemplates = allTemplates.Where(t => tokensOfType >= t.MinTokensRequired).ToList();
        ...
        List<LetterTemplate> templatesByCategory = availableTemplates
            .GroupBy(t => t.Category)
            .OrderByDescending(g => g.Key) // Premium > Quality > Basic
            .First()
            .ToList();

        LetterTemplate template = templatesByCategory[_random.Next(templatesByCategory.Count)];

        // Create personal message based on NPC and relationship level
        string message = CreatePersonalMessage(npc, letterType, tokensOfType);
    ...
    private Letter GenerateLetterFromOffer(LetterOffer offer)
    {
        ...
        List<NPC> possibleRecipients = allNPCs.Where(n => n.ID != offer.NPCId).ToList();
    ...
    private LetterOffer GetLetterOfferById(string npcId, string offerId)
    {
        ...
        if (_pendingOffers.TryGetValue(npcId, out List<LetterOffer>? offers))
        {
        ...
        List<LetterOffer> generatedOffers = GenerateNPCLetterOffers(npcId);
    ...
    public List<NPC> GetNPCsWithLetterOffers(string locationId)
    {
        ...
        List<NPC> npcsWithOffers = new List<NPC>();
    ...
    public void GeneratePeriodicOffers()
    {
        ...
        List<NPC> allNPCs = _npcRepository.GetAllNPCs();
        List<NPC> eligibleNPCs = allNPCs.Where(npc => ShouldNPCOfferLetters(npc.ID)).ToList();

        if (!eligibleNPCs.Any()) return;

        // 20% chance per eligible NPC per time block to generate an offer
        foreach (NPC? npc in eligibleNPCs)
        {
    ...
    private void GeneratePendingOfferForNPC(NPC npc)
    {
        ...
        if (!_pendingOffers.TryGetValue(npc.ID, out List<LetterOffer>? offers))
        {
        ...
        // Generate one offer
        List<LetterOffer> newOffers = GenerateNPCLetterOffers(npc.ID);
        if (newOffers.Any())
        {
    ...
    public Letter GenerateReturnLetter(NPC recipient, Letter originalLetter)
    {
        ...
        {
            returnRecipient = _npcRepository.GetById(originalLetter.SenderId);
        }

        // If no valid sender, pick a random NPC from a different location
        if (returnRecipient == null)
        {
            List<NPC> eligibleNPCs = _npcRepository.GetAllNPCs()
