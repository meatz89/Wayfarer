...
public class TokenFavorManager
{
    ...
    public List<TokenFavor> GetAvailableFavors(string npcId)
    {
        ...
        var availableFavors = new List<TokenFavor>();
        
        foreach (var favor in _favorsByNPC[npcId])
        {
            ...
            var tokensOfType = npcTokens.GetValueOrDefault(favor.RequiredTokenType, 0);
    ...
    public bool PurchaseFavor(string npcId, string favorId)
    {
        ...
        var favor = _favorsByNPC[npcId].FirstOrDefault(f => f.Id == favorId);
        if (favor == null)
            return false;
            
        // Validate availability
        var availableFavors = GetAvailableFavors(npcId);
    ...
    {
        var routeId = favor.GrantsId;
        if (string.IsNullOrEmpty(routeId))
            return false;
            
        // Use the RouteDiscoveryManager to discover the route
        var discovered = _routeDiscoveryManager.TryDiscoverRoute(routeId, npc.ID);
    ...
    private bool GrantItem(TokenFavor favor, NPC npc)
    {
        ...
        var quantity = favor.AdditionalData.GetValueOrDefault("quantity", "1");
        _messageSystem.AddSystemMessage(
    ...
    {
        var targetNpcId = favor.GrantsId;
    ...
    {
        // This would generate a special high-paying letter
        ...
        // TODO: Integrate with NPCLetterOfferService to generate special letter
        return true;
    }
    
    private bool GrantInformation(TokenFavor favor, NPC npc)
    {
        var info = favor.AdditionalData.GetValueOrDefault("information", "");
    ...
    {
        var serviceType = favor.GrantsId;
    ...
    {
        var favors = GetAvailableFavors(npcId);
        if (!favors.Any())
            return;
