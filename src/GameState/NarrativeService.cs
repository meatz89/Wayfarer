...
using Wayfarer.GameState;
    ...
    public class NarrativeService
    {
        ...
        public (string reaction, string summary) GenerateTokenGainNarrative(ConnectionType type, int count, string npcId)
        {
            ...
            var reactions = GetTokenGainReactions(type, npc.Name);
            var reaction = reactions[_random.Next(reactions.Length)];
            var summary = $"+{count} {type} connection with {npc.Name}";
        ...
        public (string morning, string letterCount, string severity) GenerateMorningLetterNarrative(int lettersGenerated, bool queueFull)
        {
            ...
            var morning = morningNarratives[_random.Next(morningNarratives.Length)];
            string letterCount;
        ...
        {
            var messages = new List<string>();
            var timeNarrative = GetTimeNarrative(currentTime);
            
            // Approach narrative
            var approachNarratives = GetApproachNarratives(npc.Name, timeNarrative);
            messages.Add(approachNarratives[_random.Next(approachNarratives.Length)]);
            
            // Offer message
            messages.Add($"\"{offer.Message}\"");
            
            // Payment hint based on trust level
            if (npcTokens >= 8)
            {
        ...
        {
            var transition = GetTimeTransitionNarrative(from, to);
            string action = null;
        ...
        public (string message, string severity) GenerateDeadlineWarning(Letter letter, int daysRemaining)
        {
            ...
            var severity = daysRemaining <= 1 ? "warning" : "info";
            var message = $"⏰ Letter from {letter.SenderName} {urgency}!";
        ...
        public (string narrative, string info) GenerateLetterChainNarrative(Letter parentLetter, Letter chainLetter)
        {
            ...
            var narrative = chainNarratives[_random.Next(chainNarratives.Length)];
            var info = $"📬 Chain letter: {chainLetter.SenderName} → {chainLetter.RecipientName}";
        ...
        {
            var unlocker = _npcRepository.GetNPCById(unlockerNpcId);
            var unlocked = _npcRepository.GetNPCById(unlockedNpcId);
        ...
        {
            var spendingNarratives = new Dictionary<string, string[]>
            {
                { "skip", new[] { 
            ...
            if (spendingNarratives.TryGetValue(action.ToLower(), out var narratives))
            {
        ...
        {
            var acceptanceNarratives = new Dictionary<string, string[]>
            {
            ...
            foreach (var key in acceptanceNarratives.Keys)
            {
                if (obligation.Name.Contains(key, StringComparison.OrdinalIgnoreCase))
                {
                    var narratives = acceptanceNarratives[key];
                    return narratives[_random.Next(narratives.Length)];
                }
            }
        ...
        {
            var forcedNarratives = new Dictionary<string, string[]>
            {
                { "Shadow", new[] {
