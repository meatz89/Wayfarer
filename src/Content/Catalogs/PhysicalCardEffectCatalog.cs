...
public static class PhysicalCardEffectCatalog
{
        ...
        int depth)
    {
        return new PhysicalBaseEffects
        {
            ...
            BaseExertionCost = CalculateExertionCost(depth)
        };
    }
    ...
    /// <summary>
    /// Calculate base Exertion cost (builder resource cost) from depth.
    /// Higher depth cards cost more Exertion to play.
    /// </summary>
    private static int CalculateExertionCost(int depth)
    {
        return depth switch
        {
            <= 2 => 0,   // Foundation: free (generate Exertion instead via GetExertionGeneration)
            <= 4 => 2,   // Standard: 2 Exertion
            <= 6 => 4,   // Advanced: 4 Exertion
            _ => 6       // Master: 6 Exertion
        };
    }
    ...
    {
        return CalculateExertionCost(depth);
    }
...
public class PhysicalBaseEffects
{
    ...
    public int BaseExertionCost { get; set; }
}
