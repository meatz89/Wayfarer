...
public static class LocationParser
{
    ...
    public static Location ConvertDTOToLocation(LocationDTO dto, GameWorld gameWorld)
    {
        ...
        // MIGRATION: Resolve Venue object reference from VenueId during parsing
        // Parser resolves dto.VenueId (string) → location.Venue (object reference)
        // Runtime code uses ONLY location.Venue (object), never VenueId lookups
        // FUTURE: Venues will be derived from hex position via HexSynchronizationService
        if (!string.IsNullOrEmpty(dto.VenueId))
        {
            location.Venue = gameWorld.Venues.FirstOrDefault(v => v.Name == dto.VenueId);
            if (location.Venue == null)
            {
                throw new InvalidOperationException($"Location '{dto.Id}' references non-existent Venue '{dto.VenueId}'");
            }
        }
