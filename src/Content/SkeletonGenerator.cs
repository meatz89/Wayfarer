...
public static class SkeletonGenerator
{
    ...
    public static NPC GenerateSkeletonNPC(string id, string source)
    {
        ...
        var personalityValues = Enum.GetValues<PersonalityType>();
        var professionValues = Enum.GetValues<Professions>();
    ...
    {
        var hash = Math.Abs(id.GetHashCode());
        var locationTypes = Enum.GetValues<LocationTypes>();
        
        var location = new Location(id, $"{GenericLocationNames[hash % GenericLocationNames.Length]} #{hash % 100}")
        {
    ...
    public static LocationSpot GenerateSkeletonSpot(string id, string locationId, string source)
    {
        ...
        var spot = new LocationSpot(id, $"{GenericSpotNames[hash % GenericSpotNames.Length]} #{hash % 100}")
        {
        ...
        var propertyValues = Enum.GetValues<SpotPropertyType>();
        var numProperties = 1 + (hash % 2);
        for (int i = 0; i < numProperties; i++)
        {
            var property = propertyValues[(hash + i * 13) % propertyValues.Length];
    ...
    public static ConversationCard GenerateSkeletonCard(string id, string source)
    {
        ...
        var connectionTypes = Enum.GetValues<ConnectionType>();
    ...
    public static bool TryGenerateSkeleton<T>(string id, string source, GameWorld gameWorld, out T skeleton) where T : class
    {
        ...
        else if (typeof(T) == typeof(LocationSpot))
        {
            if (!gameWorld.WorldState.locationSpots.Any(s => s.SpotID == id))
            {
                ...
                var locationId = source.Contains("location_") 
