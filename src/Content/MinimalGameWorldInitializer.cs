...
public class MinimalGameWorldInitializer
{
    ...
    private void CreateMinimalSpots(GameWorld gameWorld)
    {
        ...
        foreach (var location in gameWorld.WorldState.locations)
        {
            // Ensure each location has at least one spot
            var existingSpots = gameWorld.WorldState.locationSpots
            ...
            if (!existingSpots.Any())
            {
                ...
                var defaultSpot = spotFactory.CreateMinimalSpot($"{location.Id}_square", location.Id);
    ...
    private void CreateMinimalNPCs(GameWorld gameWorld)
    {
        ...
        foreach (var location in gameWorld.WorldState.locations)
        {
            var existingNPCs = gameWorld.WorldState.NPCs
...
Size = SizeCategory.Small
    ...
    private void CreateMinimalRoutes(GameWorld gameWorld)
    {
        ...
        for (int i = 0; i < locations.Count - 1; i++)
        {
            for (int j = i + 1; j < locations.Count; j++)
            {
                ...
                var existingRoute = gameWorld.WorldState.Routes
                ...
                if (!existingRoute)
                {
                    ...
                    var reverseRoute = new RouteOption
                    {
    ...
    private void TryLoadLocationsFromJson(GameWorld gameWorld)
    {
        try
        {
            var path = Path.Combine(_contentDirectory.Path, "Templates", "locations.json");
            if (File.Exists(path))
            {
