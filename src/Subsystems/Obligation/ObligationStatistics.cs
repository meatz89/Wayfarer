...
public class ObligationStatistics
{
    ...
    public QueueActivity GetQueueActivity()
    {
        ...
        foreach (LetterHistoryEntry npcHistory in player.NPCLetterHistory)
        {
            LetterHistory history = npcHistory.History;
    ...
    public QueueSnapshot CreateQueueSnapshot()
    {
        ...
        return new QueueSnapshot
        {
            ...
            Meetings = [.. player.MeetingObligations],
    ...
    private SatchelInfo GetSatchelAnalysis()
    {
        ...
        info.FragileLetters = [.. info.CarriedLetters.Where(l => l.PhysicalProperties.HasFlag(LetterPhysicalProperties.Fragile))];

        info.PerishableLetters = [.. info.CarriedLetters.Where(l => l.PhysicalProperties.HasFlag(LetterPhysicalProperties.Perishable))];

        info.ValuableLetters = [.. info.CarriedLetters.Where(l => l.PhysicalProperties.HasFlag(LetterPhysicalProperties.Valuable))];
    ...
    private List<string> GetMostActiveNPCs(int count)
    {
        ...
        return player.NPCLetterHistory
            .OrderByDescending(h => h.History.DeliveredCount + h.History.ExpiredCount + h.History.SkippedCount)
            .Take(count)
            .Select(h => GetNPCNameById(h.NpcId))
            .ToList();
    }
    ...
    private List<string> GetMostProblematicNPCs(int count)
    {
        Player player = _gameWorld.GetPlayer();
        return player.NPCLetterHistory
            .Where(h => h.History.ExpiredCount > 0)
            .OrderByDescending(h => h.History.ExpiredCount)
            .Take(count)
            .Select(h => GetNPCNameById(h.NpcId))
            .ToList();
    }
