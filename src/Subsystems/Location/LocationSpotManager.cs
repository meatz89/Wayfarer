...
namespace Wayfarer.Subsystems.LocationSubsystem
{
    ...
    public class LocationSpotManager
    {
        ...
        public LocationSpot FindSpotInLocation(Location location, string spotIdentifier)
        {
            ...
            if (targetSpot == null)
            {
                var spotsInLocation = GetSpotsForLocation(location.Id);
            ...
            NPCRepository npcRepository)
        {
            ...
            var spots = GetSpotsForLocation(location.Id);
            ...
            foreach (var spot in spots)
            {
                ...
                var npcsAtSpot = npcRepository.GetNPCsForLocationSpotAndTime(spot.SpotID, currentTime);
        ...
        private string GenerateSpotDetail(LocationSpot spot)
        {
            ...
            var descGenerator = new SpotDescriptionGenerator();
            var currentTime = TimeBlocks.Morning; // Default for brief descriptions
        ...
        {
            var activeProperties = GetActiveSpotProperties(spot, timeBlock);
        ...
        {
            var spots = GetSpotsForLocation(locationId);
        ...
        {
            var location = _locationManager.GetLocation(locationId);
            if (location == null) return null;
            
            // First try to get the designated travel hub
            if (!string.IsNullOrEmpty(location.TravelHubSpotId))
            {
                var travelHub = _locationManager.GetSpot(locationId, location.TravelHubSpotId);
                if (travelHub != null) return travelHub;
            }
            
            // Then look for spots tagged as crossroads
            var spots = GetSpotsForLocation(locationId);
            var crossroads = spots.FirstOrDefault(s => s.DomainTags?.Contains("Crossroads") == true);
        ...
        {
            var spots = GetSpotsForLocation(locationId);
            var serviceSpots = new List<LocationSpot>();
            
            foreach (var spot in spots)
            {
                // Check if spot has properties that indicate this service
                var properties = GetActiveSpotProperties(spot, timeBlock);
