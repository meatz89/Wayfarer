...
{
    // ... existing properties
    public List<CustomContentDTO> CustomContent { get; set; }
}
```

3. Add loader method in PackageLoader:
```csharp
private void LoadCustomContent(List<CustomContentDTO> contentDtos)
{
    foreach (var dto in contentDtos)
    {
        // Parse and add to GameWorld
    }
}
```
...
- `NPCParser.cs` - Validates and parses NPCs
- `LocationParser.cs` - Validates and parses locations
