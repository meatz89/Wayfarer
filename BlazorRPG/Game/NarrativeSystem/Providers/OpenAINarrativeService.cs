public class OpenAINarrativeService : BaseNarrativeAIService
{
        ...
        : base(new OpenAIProvider(configuration, logger), configuration, narrativeLogManager)
    {
        ...
        _model = configuration.GetValue<string>("OpenAI:Model") ?? "o4-mini";
    }
    ...
    {
        string conversationId = $"{context.LocationName}_o4mini"; // Consistent ID with o4 marker
    ...
    public override async Task<Dictionary<CardDefinition, ChoiceNarrative>> GenerateChoiceDescriptionsAsync(
        ...
        WorldStateInput worldStateInput)
    {
        string conversationId = $"{context.LocationName}_o4mini";
        string systemMessage = _promptManager.GetSystemMessage(worldStateInput);
    ...
    public override async Task<string> GenerateActionsAsync(
        ...
        WorldStateInput worldStateInput)
    {
        string conversationId = $"action_generation_{context.SpotName}_o4mini";
        string systemMessage = _promptManager.GetSystemMessage(worldStateInput);
