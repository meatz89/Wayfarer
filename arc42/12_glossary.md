...
| Term | Definition |
|------|------------|
| **ArchetypeIntensity** | Player readiness filter for procedural content: Peaceful (recovery), Building (accumulation), Testing (moderate challenge), Crisis (resource test). Determines WHICH archetypes are safe for current player state. Orthogonal to RhythmPattern. |
